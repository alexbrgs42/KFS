...
/// @brief Initializes the terminal by initializing its associated global
/// variables
// 			and displaying the space char for every characters to
// display an empty terminal.
/// @param  None
...
void clear_and_prepare_buffers(void) {
    ...
    // For each window, clear the command buffer and print the info line and the
    // prompt
    switch_window(2);
    clear_command_buffer();
    print_info_line();
...
/// @brief Prints the info line at the bottom of the screen indicating which
/// window we're on
